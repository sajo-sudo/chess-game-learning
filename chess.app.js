(function(exports) {
  const coinIcons = {
   ...
   <g>
    ...
    Biship: `<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
   viewBox="0 0 297.08 297.08" style="enable-background:new 0 0 297.08 297.08;" xml:space="preserve">
<g>
...
<g>
    ...
    Knight: `<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
viewBox="0 0 298 298" style="enable-background:new 0 0 298 298;" xml:space="preserve">
<g>
...
<g>
    ...
    Bishop: `<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
viewBox="0 0 297.08 297.08" style="enable-background:new 0 0 297.08 297.08;" xml:space="preserve">
<g>
<path d="M206.873,255.08h-3.41c2.214-3.337,8.32-14.536-0.712-25.6c-8.9-10.905-25.137-39.546-24.448-64.4h3.57
 c4.418,0,7.667-3.582,7.667-8v-1c0-4.418-3.249-8-7.667-8h-4.333v-4.285c13-8.971,20.511-23.502,20.511-39.914
...
<g>
    ...
    King: ` <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
viewBox="0 0 298 298" style="enable-background:new 0 0 298 298;" xml:space="preserve">
<g>
...
<g>
    ...
    Queen: ` <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
viewBox="0 0 297.77 297.77" style="enable-background:new 0 0 297.77 297.77;" xml:space="preserve">
<g>
...
<g>
    ...
    sippai: `<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
...
<g>
  ...
  function squareClickListener(event) {
    ...
    const coin = chessBoard.getCoinOnpos([this.row, this.col]); // get the coin on current clicked square
    if (coin) {
      // if the coin presents, we need select the coin
      if (chessBoard.selectedCoin) {
        // if a coin already selected, then, we need unselect that square
        const pos = chessBoard.getPosOfCoin(chessBoard.selectedCoin);
        getChessSquare(pos).classList.remove("selected_coin_square");
      }
      // ************************ select the square here ************************//
      this.classList.add("selected_coin_square");
      const possibleMovPos = chessBoard.getPosibleMoves([this.row, this.col]); // [pos]
      console.log("possibleMovPos ::: ", possibleMovPos);
      console.log(
  ...
  function createCol(isBlaskIsStart, row) {
    ...
    for (let i = 0; i < 8; i++) {
      col.appendChild(createSquare(currentColor, row, i));
  ...
  function getChessSquare([row, col]) {
    return document.getElementById("row-" + row + "-col-" + col);
  }

  function putCoinOnPos(pos, coin, isBlack) {
    const square = getChessSquare(pos);
    square.innerHTML = coinIcons[coin];
    if (isBlack) {
      square.classList.add("black_coin");
    } else {
  ...
  function renderChessPos(coins, isBlack) {
    for (const coin in coins) {
      if (singleCoins.indexOf(coin) !== -1) {
        putCoinOnPos(coins[coin], coin, isBlack);
      } else if (doubleCoins.indexOf(coin) !== -1) {
        putCoinOnPos(coins[coin].Left, coin, isBlack);
        putCoinOnPos(coins[coin].Right, coin, isBlack);
      } else {
        coins[coin].forEach(pos => putCoinOnPos(pos, coin, isBlack));
      }
    }
  }

  function createChessBoardWithInitialPos(chessBoard) {
    renderChessPos(chessBoard.coins.white, false);
    renderChessPos(chessBoard.coins.black, true);
  }

  const container = document.getElementById("container");
  let isBlack = false;
  for (let i = 0; i < 8; i++) {
    container.appendChild(createRow(isBlack, i));
    isBlack = !isBlack;
  }
